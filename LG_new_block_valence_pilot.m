...
close all;
clear;
%% pseudo randomize trial
%load("sequence_valence1.mat") %subj 8,9
%sequence=1;
%load("sequence_valence2.mat")
%sequence=2;

load("sequence_valence3.mat") %subj 10,11
sequence=3;

%load("sequence_valence4.mat")
%sequence=4;


%% change path to be here
current_path=pwd;
% data_path=[current_path,'/pilot_Data/'];
% data_path=[current_path,'/pilot_Data/old/'];
dazta_path=[current_path,'/data_valence/'];
%% EEG trigger open
...
task_id = 'LG_valence_pilot';%newest_datafile(1:3);
subj_ID = 100;
...
%     
%     
%     edf_file_name = 'va19';%input('Set the file name of eye-tracking data: ');
%     fprintf('EDFFile: %s\n', edf_file_name );
%     
...
screenShot=load('valence_rating.mat'); %load valence, valence Rating images
...
if dummymode == 0
    FileName = ['LG_',num2str(subj_ID),'_valence_pilot_EEG.mat'];
elseif dummymode == 1
    FileName = ['LG',num2str(subj_ID),'_valence_pilot.mat'];
end
