...
%load("sequence_vivid1.mat") 
%sequence=1;

%load("sequence_vivid2.mat")
%sequence=2;

%%load("sequence_vivid3.mat") 
sequence=3;

%load("sequence_vivid4.mat")
%sequence=4;
...
task_id = 'Behavior_vivid';%newest_datafile(1:3);
subj_ID = 100;
...
screenShot=load('vivid_rating.mat'); %load vivid, vivid Rating images
...
% confirm_data(i,3:6) = trial_info; [vivid, boosting, color_gamut,image_number]
%% Start
...
for trial=1:length(new_row_all)
    ...
    %% image 3s show
    %upload image without 3 continuous present with same vivid or same boosting option
...
if dummymode == 0
    FileName = ['Behavior_vivid',num2str(subj_ID),'.mat'];
elseif dummymode == 1
    FileName = ['Behavior_vivid',num2str(subj_ID),'.mat'];
end
