...
%load("sequence_vivid1.mat") %Subj 8,9
%sequence=1;
%load("sequence_vivid2.mat") %SUBJ 6,7
%sequence=2;
load("sequence_vivid3.mat") %Subj 10,11
sequence=3;
%load("sequence_vivid4.mat") %Subj 4,5
%sequence=4;
%save("New_sequence.mat","trial_info","new_row")
%saccxxxxzcve("New_sequence_threetimes_replicated.mat","trial_info_all","new_row_all")
% Clear tche workspace and the screen


%% change path to be herexcxxxx
current_path=pwd;
% data_path=[current_path,'/pilot_Data/'];
% data_path=[current_path,'/pilot_Data/old/'];
data_path=[current_path,'/data_vivid/'];
%% EEG trigger open
...
task_id = 'LG_vivid';%newest_datafile(1:3);
subj_ID = 100;
...
screens = Screen('Screens');%screen이 2개기 때문에, screen number를 1또는 2로 지정해줘야 한다.
...
if eyelinkOn
    ...
    edf_file_name = 'vi19';%input('Set the file name of eye-tracking data: ');
    fprintf('EDFFile: %s\n', edf_file_name );
...
%% image load
%load("New_sequence_replicated.mat")%row for the IAPS image and trial info
load("IAPS_image_name_replicated.mat")%load name
screenShot=load('vivid_rating.mat'); %load valence, arousal Rating images
...
confirm_data = nan(nT, 3);
% confirm_data_t(i,1) = fixation_t1;
% confirm_data_t(i,2) = image_start;
% confirm_data_t(i,3) = fixation_t2;
% confirm_data_t(i,4) = rating_start;
% confirm_data_t(i,5) = rating_end;
...
if dummymode == 0
    FileName = ['LG_',num2str(subj_ID),'_vivid_EEG.mat'];
elseif dummymode == 1
    FileName = ['LG',num2str(subj_ID),'_vivid.mat'];
end


save([data_path, FileName],'subj_ID','ExpName','confirm_data','confirm_data_t','sequence');%,'confirm_bonus','confirm_bonus_gamble');

data_backup='C:\Users\unist\OneDrive\바탕 화면\data_backup';
%save([data_backup, FileName],'subj_ID','ExpName','confirm_data','confirm_data_t','sequence');%,'confirm_bonus','confirm_bonus_gamble');
