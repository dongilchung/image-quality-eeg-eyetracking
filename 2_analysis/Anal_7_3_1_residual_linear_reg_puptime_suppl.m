...
for s = 1:length(sList)
    for vr = 1:length(varList)
        ...
        % initial fitting with arousal and valence rating
        curr_m1 = fitlm(Xdat(:,2:3), regdatTab_vi{idx, varList(vr)});
        % residual fitting with vividness rating
        curr_m2 = fitlm(Xdat(:,1), curr_m1.Residuals.Raw);
        estimates = [curr_m1.Coefficients.Estimate(1:3); curr_m2.Coefficients.Estimate(1:2)];
        regTab_vi = [regTab_vi; sList(s) varList(vr) estimates'];
        
        % initial fitting with arousal and valence rating
        curr_m1 = fitlm(Xdat(:,2:3), regdatTab_ar{idx, varList(vr)});
        % residual fitting with vividness rating
        curr_m2 = fitlm(Xdat(:,1), curr_m1.Residuals.Raw);
        estimates = [curr_m1.Coefficients.Estimate(1:3); curr_m2.Coefficients.Estimate(1:2)];
        regTab_ar = [regTab_ar; sList(s) varList(vr) estimates'];
        
        % initial fitting with arousal and valence rating
        curr_m1 = fitlm(Xdat(:,2:3), regdatTab_va{idx, varList(vr)});
        % residual fitting with vividness rating
        curr_m2 = fitlm(Xdat(:,1), curr_m1.Residuals.Raw);
        estimates = [curr_m1.Coefficients.Estimate(1:3); curr_m2.Coefficients.Estimate(1:2)];
        regTab_va = [regTab_va; sList(s) varList(vr) estimates'];
    end
end
...
regTab_vi.Properties.VariableNames = {'sub', 'var', ...
    'intercept1', 'beta_ar', 'beta_va',...
    'intercept2', 'beta_vi'};
...
save('pupilTime_residual_reg.mat','-struct','tabs')
